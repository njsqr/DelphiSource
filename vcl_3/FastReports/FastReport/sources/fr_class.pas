...
function TfrView.DoMethod(MethodName: string; Par1, Par2, Par3: Variant):
  Variant;
...
begin
  ...
  gapx := wx2 + Round(gapx * ScaleX);
  gapy := wy2 div 2 + Round(gapy * ScaleY);
...
begin
  ...
  if DocMode = dmDesigning then
    ...
    Canvas.FillRect(Rect(x, y,
      ...
      Round((SaveX + SaveDX) * ScaleX + OffsX), Round((SaveY + SaveDY) * ScaleY
        + OffsY)));
...
var
  ...
  begin
    ...
    if s.Count > 0 then
      ...
      Result := TfrView(s.Objects[s.Count - 1])
    else
...
begin
  ...
  begin
    ...
    begin
      ...
      if (t = nil) or not (TfrBandView(t).BandType in
...
begin
  ...
  if (BandAlign = baWidth) and (CurBand <> nil) and (CurBand.Typ in
    [btCrossHeader..btCrossFooter]) then
...
function TfrStretcheable.DoMethod(MethodName: string; Par1, Par2, Par3:
  Variant): Variant;
...
begin
  ...
  if Index = 'ALIGNMENT' then
    ...
    Flags := (Flags and not flSuppressRepeated) or Word(Boolean(Value)) *
      flSuppressRepeated
...
function TfrMemoView.DoMethod(MethodName: string; Par1, Par2, Par3: Variant):
  Variant;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                (WCanvas.TextWidth(Copy(s, beg, last - beg + 1 + Ord(b[i])) +
                  '-') <= maxwidth) do
            ...
            if ((Flags and flWordBreak) <> 0) and not WasBreak and (last = cur -
              1) then
      ...
      begin
        ...
        begin
          ...
          if Alignment = 0 then
            ...
            curx := x + {dx}(DRect.Right - DRect.Left) - 1 - gapx -
              Canvas.TextWidth(str)

          else

            curx := x + gapx + ({dx}(DRect.Right - DRect.Left) - gapx - gapx -
          ...
          begin
            ...
            if (n <> 0) and not ParaEnd then
              ...
              SetTextJustification(Canvas.Handle,
                ...
                {dx}(DRect.Right - DRect.Left) - gapx - gapx -
                  Canvas.TextWidth(str), n);
  ...
  var
    ...
    begin
      ...
      if Alignment = 4 then
        ...
        ({dy}(DRect.Bottom - DRect.Top - gapy - gapy) - Canvas.TextWidth(str))
          div 2
      ...
      begin
        ...
        if (n <> 0) and not ParaEnd then
          ...
          SetTextJustification(Canvas.Handle,
            ...
            {dy}(DRect.Bottom - DRect.Top) - gapy - gapy -
              Canvas.TextWidth(str), n);
...
begin
  ...
  else if (Alignment and $4) <> 0 then
    OutMemo90
  else
    OutMemo;
...
begin
  ...
  Result := (dy - Round(FrameWidth * 2) - gapy * 2) mod TextHeight + gapy * 2 +
    Round(FrameWidth * 2);
  ...
  begin
    ...
    begin
      ...
      if t.Selected and ((t.Restrictions and frrfDontModify) = 0) then
        ...
        t.Flags := (t.Flags and not flSuppressRepeated) + Word(Checked) *
          flSuppressRepeated;
  ...
  begin
    ...
    begin
      ...
      if t.Selected and ((t.Restrictions and frrfDontModify) = 0) then
        ...
        t.Flags := (t.Flags and not flUnderlines) + Word(Checked) *
          flUnderlines;
...
var
  ...
  begin
    ...
    begin
      ...
      (TfrBandType(t.FrameTyp) in [btMasterData, btDetailData, btSubDetailData])
        then
...
begin
  ...
  begin
    ...
    Flags := (Flags and not flBandNewPageAfter) or Word(Boolean(Value)) *
      flBandNewPageAfter

  else if Index = 'PRINTIFSUBSETEMPTY' then

    Flags := (Flags and not flBandPrintIfSubsetEmpty) or Word(Boolean(Value)) *
  ...
  begin
    ...
    Flags := (Flags and not flBandBreaked) or Word(Boolean(Value)) *
      flBandBreaked;
    ...
    Flags := (Flags and not flBandOnFirstPage) or Word(Boolean(Value)) *
      flBandOnFirstPage

  else if Index = 'ONLASTPAGE' then

    Flags := (Flags and not flBandOnLastPage) or Word(Boolean(Value)) *
      flBandOnLastPage

  else if Index = 'REPEATHEADER' then

    Flags := (Flags and not flBandRepeatHeader) or Word(Boolean(Value)) *
      flBandRepeatHeader

  else if Index = 'PRINTCHILDIFINVISIBLE' then

    Flags := (Flags and not flBandPrintChildIfInvisible) or Word(Boolean(Value))
...
function TfrBandView.DoMethod(MethodName: string; Par1, Par2, Par3: Variant):
  Variant;
...
{$ENDIF}
    ...
    begin
      ...
      for i := 1 to Columns do
        ...
        Rectangle(frDesigner.Page.LeftMargin + (i - 1) * (ColumnWidth +
          ColumnGap), y,

          frDesigner.Page.LeftMargin + (i - 1) * (ColumnWidth + ColumnGap) +
            ColumnWidth, y + dy);
    ...
    if ShowBandTitles then
      ...
      begin
        ...
        MoveTo(x - 18, y + RWidth - 2);
        LineTo(x, y + RWidth - 2);

        Pen.Color := clBlack;

        MoveTo(x - 18, y + RWidth - 1);
        LineTo(x, y + RWidth - 1);
        ...
        TextOut(x - 15, y + TextWidth(frBandNames[FrameTyp]) + 4,
      ...
      begin
        ...
        MoveTo(x + RWidth - 2, y - 18);
        LineTo(x + RWidth - 2, y);

        Pen.Color := clBlack;

        MoveTo(x + RWidth - 1, y - 18);
        LineTo(x + RWidth - 1, y);
  ...
  begin
    ...
    begin
      ...
      if t.Selected and ((t.Restrictions and frrfDontModify) = 0) then
        ...
        t.Flags := (t.Flags and not flBandBreaked) + Word(Checked) *
          flBandBreaked;
...
begin
  ...
  begin
    ...
    if (Restrictions and frrfDontModify) = 0 then
      ...
      Flags := (Flags and not flBandOnFirstPage) + Word(Checked) *
        flBandOnFirstPage;
...
begin
  ...
  begin
    ...
    if (Restrictions and frrfDontModify) = 0 then
      ...
      Flags := (Flags and not flBandOnLastPage) + Word(Checked) *
        flBandOnLastPage;
...
begin
  ...
  begin
    ...
    if (Restrictions and frrfDontModify) = 0 then
      ...
      Flags := (Flags and not flBandRepeatHeader) + Word(Checked) *
        flBandRepeatHeader;
...
begin
  ...
  begin
    ...
    if (Restrictions and frrfDontModify) = 0 then
      ...
      Flags := (Flags and not flBandPrintChildIfInvisible) + Word(Checked) *
        flBandPrintChildIfInvisible;
...
begin
  ...
  AddProperty('DataField', [frdtOneObject, frdtHasEditor, frdtString],
    frFieldEditor);
...
function TfrPictureView.DoMethod(MethodName: string; Par1, Par2, Par3: Variant):
  Variant;
...
begin
  ...
  begin
    ...
    if ((Picture.Graphic = nil) or Picture.Graphic.Empty) and (DocMode =
      dmDesigning) then
  ...
  try
    ...
    if t.Parent = Self then
      ...
      begin
        ...
        ox := t.x;
        Inc(t.x, Parent.XAdjust - Parent.LeftMargin + Parent.LeftOffset);
    ...
    begin
      ...
      if Parnt.Objects.IndexOf(t) <> -1 then
        ...
        begin
          ...
          sfx := t.x;
          Inc(t.x, CurX + Parent.LeftOffset);
...
begin
  ...
  if not Parent.BandExists(Self) then
    Exit;
  ...
  begin
    ...
    begin
      ...
      Parent.XAdjust := SaveXAdjust + (CurColumn - 1) * (View.ColumnWidth +
        View.ColumnGap);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if not ((CHeight > Parent.CurBottomY - Parent.TopMargin) and
            (Parent.CurY = Parent.TopMargin)) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        sx := (Pw - (Left + Right)) / Pgw;
        sy := (Ph - (Top + Bottom)) / Pgh;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                ShellExecute(GetDesktopWindow, nil, PChar(t.Tag), nil, nil,
                  sw_ShowNormal)
...
begin
  ...
  begin
    ...
    if t is TfrMemoView then
      ...
      TfrMemoView(t).HighlightStr :=
        frParser.Str2OPZ(TfrMemoView(t).HighlightStr);
  ...
  begin
    ...
    if b in [btCrossHeader..btCrossFooter] then
      continue;
  ...
  if BandExists(Bands[btCrossData]) and (Pos(';',
    Bands[btCrossData].View.FormatStr) <> 0) then
        ...
        begin
          ...
          if Bnd.View <> nil then
            ...
            if AnsiCompareText(Bnd.View.Name, Copy(s, i, j - i)) = 0 then
              ...
              CreateDS(Copy(s, j + 1, n - j - 1), Bnd.VCDataSet,
                Bnd.IsVirtualVCDS);
  ...
  ColWidth := (RightMargin - LeftMargin - ((ColCount - 1) * ColGap)) div
    ColCount;
...
begin
  ...
  if (PageType = ptDialog) or (MasterReport.DoublePass and
  ...
  try
    ...
    begin
      ...
      begin
        ...
        case Bnd.Typ of
          ...
          btSubdetailData:
            ...
            AddAggregate([btPageFooter, btColumnFooter, btMasterFooter,
              ...
              btDetailFooter, btSubdetailFooter, btGroupFooter,
                btReportSummary]);
        ...
        if (Bnd = nil) or (Bnd.DataSet = nil) then
          break;
...
begin
  ...
  if (PageNo <> 0) or ((Bands[btPageFooter].Flags and flBandOnFirstPage) <> 0)
    then
...
{$ENDIF}
              ...
              begin
                ...
                if b1.NewPageAfter and b1.Visible then
                  NewPage;
          ...
          begin
            ...
            if not b.DataSet.Eof and b.NewPageAfter and b.Visible then
              NewPage;
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((PageNo = 0) and ((Bands[btPageFooter].Flags and flBandOnFirstPage) <>
        0)) or
...
begin
  ...
  begin
    ...
    AddEnumProperty('BorderStyle', 'bsDialog;bsSizeable', [bsDialog,
      bsSizeable]);
    ...
    AddEnumProperty('Position', 'poDesigned;poScreenCenter', [poDesigned,
      poScreenCenter]);
...
begin
  ...
  if (Variables.Count > 0) and (Variables.Name[0] <> '') and
    (Variables.Name[0][1] <> ' ') then
...
begin
  ...
  Filer.DefineBinaryProperty('ReportForm', ReadBinaryData, WriteBinaryData,
    True);
...
begin
  ...
  begin
    ...
    if not Assigned(FDFMStream) then
      Exit;
...
begin
  ...
  with View do
    ...
    if (FDataSet <> nil) and frIsBlob(TfrTField(FDataSet.FindField(FField)))
      then
...
procedure TfrReport.InternalOnPrintColumn(ColNo: Integer; var ColWidth:
  Integer);
      ...
      1:
        ...
        begin
          ...
          case f2 of
            ...
            0: Result := FormatFloat('##0.' + Dup('#', (Format div $0100) and
              $FF), v);

            1: Result := FloatToStrF(v, ffFixed, 15, (Format div $0100) and
              $FF);

            2: Result := FormatFloat('#,##0.' + Dup('#', (Format div $0100) and
              $FF), v);

            3: Result := FloatToStrF(v, ffNumber, 15, (Format div $0100) and
              $FF);
...
{$ENDIF}
              ...
              raise(EParserError.Create(frLoadStr(SUndefinedSymbol) + ' "' +
                SubValue + '"'));
...
procedure TfrReport.GetVariableV(const s: string; var v: Variant);
  ...
  begin
    ...
    (Name = 'PAGEHEIGHT') or (Name = 'PAGEWIDTH') then
      Value := '0';
...
const
  Signature: array[0..2] of Byte = ($F0, $C0, $00);
...
{$ENDIF}
    ...
    begin
      ...
      if not ((PassForm.ShowModal = mrOk) and (PassForm.E1.Text =
        ReportPassword)) then
    ...
    begin
      ...
      Application.MessageBox(PChar(frLoadStr(SFRFError)),
        PChar(frLoadStr(SError)),
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      if Assigned(r.FOnBeginDoc) then
        r.FOnBeginDoc;
...
var
  ...
  begin
    ...
    if (ds <> nil) and (ds is TfrDBDataSet) then
      ...
      frDataManager.PrepareDataSet(TfrTDataSet((ds as
        TfrDBDataSet).GetDataSet));
...
begin
  ...
  begin
    ...
    if not MasterReport.DoublePass or not MasterReport.FinalPass then
      ...
      if Assigned(FOnCrossBeginDoc) then
        FOnCrossBeginDoc;
    ...
    repeat
      ...
      begin
        ...
        begin
          ...
          if Skip or (PageType = ptDialog) or not Visible then
            continue;
        ...
        if ((i = Pages.Count - 1) and CompositeMode and (not b or Dataset.Eof))
          or
...
begin
  ...
  if (EMFPages = nil) or (Printer.Printers.Count = 0) then
    Exit;
...
var
  ...
  begin
    ...
    if not ((PrintPages = frAll) or
      ...
      ((PrintPages = frEven) and ((n + 1) mod 2 = 0))) then
      Exit;
    ...
    begin
      ...
      with PrnInfo, pgMargins do
        ...
        if UseMargins then
          ...
          EMFPages.Draw(n, Printer.Canvas, Rect(-POfx, -POfy, PPgw - POfx, PPgh
            - POfy))
      ...
      begin
        ...
        begin
          ...
          ((EMFPages[EMFPages.Count - 1].pgOr <> NextDoc.Pages[0].pgOr) or
            ...
            (EMFPages[EMFPages.Count - 1].pgSize <> NextDoc.Pages[0].pgSize))
              then
        ...
        begin
          ...
          if Doc.Pages.Count > 0 then
            ...
            ((Doc.Pages[Doc.Pages.Count - 1].pgOr <> NextDoc.Pages[0].pgOr) or
              ...
              (Doc.Pages[Doc.Pages.Count - 1].pgSize <> NextDoc.Pages[0].pgSize))
                then
...
procedure TfrFunctionLibrary.AddFunctionDesc(FuncName, Category, Description:
  string);
...
procedure TfrInstalledFunctions.UnRegisterFunctionLibrary(FunctionLibrary:
  TfrFunctionLibrary);
...
begin
  ...
  begin
    ...
    if (p.FunctionLibrary <> nil) and (p.FunctionLibrary.ClassName =
      FunctionLibrary.ClassName) then
...
begin
  ...
  FIDEMode := AnsiUpperCase(ExtractFileName(Application.ExeName)) =
    'DELPHI32.EXE';
...
begin
  ...
  for i := 0 to 41 do
    ...
    if i <> 16 then
      ...
      frConsts[frColorNames[i]] := frColors[i]
    else
...
var
  ...
  rsAggregate, rsDateTime, rsString, rsOther, rsInfo, rsMath, rsBool, rsInterpr:
    string;
...
begin
  ...
  AddFunctionDesc('FORMATDATETIME', rsString,
    frLoadStr(SDescriptionFORMATDATETIME));
    ...
    15:
      ...
      begin
        ...
        if Length(s) > 0 then
          ...
          val := AnsiUpperCase(s[1]) + Copy(s, 2, Length(s) - 1)
        else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        AggrBand.AddAggregateValue(s + #1 + Chr(at) + #1 + p1 + #1 + p3 + #1,
          v);
    ...
    begin
      ...
      val := b.AggrBand.GetAggregateValue(s + #1 + Chr(at) + #1 + p1 + #1 + p3 +
        #1);
...
begin
  ...
  //  AddFunctionDesc('FIELDISNULL', frLoadStr(frRes + 13872), frLoadStr(frRes + 13872));

  AddFunctionDesc('FIELDISNULL', frLoadStr(frRes + 2620), frLoadStr(frRes +
    2621));
