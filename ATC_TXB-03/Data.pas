...
begin
 ...
 // filename1:=ExtractFilePath(ParamStr(0)) + 'MDB\lhq.xls';
  filename1:=ExtractFilePath(ParamStr(0)) + 'MDB\零件库存.MDB';
  ...
  if  FileExists(filename2) then
    begin
      ...
      if  FileExists(filename1) then
        begin
          //ADOConnection1.ConnectionString:='Provider=MSDASQL.1;Persist Security Info=False;Mode=read;Data Source=Excel Files;Initial Catalog='
         // +filename1+';';
        ...
        begin
          MessageBox(0,'库存表文件“lhq.xls” 找不到！', '奥特佳特性表系统', MB_OK);
          flag:=0;
        end;
    end
  else
