...
begin
  ...
  //年月日
  code:=code+y_Arr[y-2000-10]+m_Arr[m-1]+d_Arr[d-1];
  if combobox1.ItemIndex=-1 then
    begin
      showmessage('生产车间不能为空！！！'+#13+#13+'请重新选择车间代号...');
      combobox1.SetFocus
    end
  else
    begin
      if (MessageBox(Handle,'生产车间及班次确认了么？','提示',MB_YESNO or MB_ICONwarning)=7) then
        Combobox1.SetFocus
      else
        begin
          ...
          for i:=1 to 13 do
            begin
              for j:=0 to 36 do
                begin
                  if str[i]=xy_Arr[j] then
                    flag:=flag+j;
                end;
            end;
          flag:=flag mod 37;
          edit5.text:=xy_Arr[flag];//校验码
          code:=code+xy_Arr[flag];
          //返工码
